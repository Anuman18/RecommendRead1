...
def bookmark_story(story_id):
    from models import Bookmark, Story  # import inside function
    ...
    try:
        ...
        existing_bookmark = Bookmark.query.filter_by(user_id=user_id, story_id=story_id).first()
        if existing_bookmark:
            return jsonify({'message': 'Story already bookmarked', 'bookmark': existing_bookmark.to_dict()}), 200
        
        new_bookmark = Bookmark(user_id=user_id, story_id=story_id)
        db.session.add(new_bookmark)
        db.session.commit()
        
        return jsonify({'message': 'Story bookmarked successfully', 'bookmark': new_bookmark.to_dict()}), 201
...
def remove_bookmark(story_id):
    from models import Bookmark  # import inside function

    user_id = session.get('user_id')
    
    try:
        bookmark = Bookmark.query.filter_by(user_id=user_id, story_id=story_id).first()
        if not bookmark:
